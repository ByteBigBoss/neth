...
const ContactDetailsCard = ({ title, value, displayValue, icon, copy = false }: ContactDetailsCardProps) => {
  const [message, setMessage] = useState('Hello I am nethmina,');
  ...
  return (
    <div className='w-full border p-[12px] rounded-[12px] bg-[#fff] dark:bg-[#1A1A1A] min-h-[74px] flex gap-[15px] items-center relative'>
      ...
      <div className='flex flex-col gap-1'>
        ...
        <div className='text-[12px] font-medium opacity-80'>{displayValue?displayValue:value}</div>
      </div>

      {copy &&
        <Button onClick={handleCopy} className='w-[34px] group min-w-[34px] flex justify-center items-center h-[30px] border rounded-[12px] bg-[#fafafa] dark:bg-[#232323] absolute top-[10px] right-[10px]'>
